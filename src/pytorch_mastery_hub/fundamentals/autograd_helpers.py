# pytorch_mastery_hub/fundamentals/autograd_helpers.py
"""
...
def gradient_check(
    func: Callable, inputs: torch.Tensor, eps: float = 1e-6, atol: float = 1e-5, rtol: float = 1e-3
) -> bool:
    """
...
def compute_jacobian(
    outputs: torch.Tensor, inputs: torch.Tensor, create_graph: bool = False
) -> torch.Tensor:
    """
...
def compute_hessian(output: torch.Tensor, inputs: torch.Tensor) -> torch.Tensor:
    """
    ...
    first_grads = torch.autograd.grad(
        outputs=output, inputs=inputs, create_graph=True, retain_graph=True
    )[0].view(-1)
    ...
    for i in range(input_dim):
        if first_grads[i].requires_grad:
            second_grads = torch.autograd.grad(
                outputs=first_grads[i], inputs=inputs, retain_graph=True, allow_unused=True
            )[0]
