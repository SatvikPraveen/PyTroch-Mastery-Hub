# pytorch_mastery_hub/fundamentals/math_utils.py
"""
...
def cross_entropy(
    pred: torch.Tensor, target: torch.Tensor, reduction: str = "mean", label_smoothing: float = 0.0
) -> torch.Tensor:
    """
...
def huber_loss(
    pred: torch.Tensor, target: torch.Tensor, delta: float = 1.0, reduction: str = "mean"
) -> torch.Tensor:
    """
...
def normalize(x: torch.Tensor, p: float = 2.0, dim: int = -1, eps: float = 1e-12) -> torch.Tensor:
    """
...
def standardize(x: torch.Tensor, dim: int | None = None, eps: float = 1e-8) -> torch.Tensor:
    """
...
def jensen_shannon_divergence(p: torch.Tensor, q: torch.Tensor) -> torch.Tensor:
    ...
    """
    m = 0.5 * (p + q)
    js = 0.5 * F.kl_div(p.log(), m, reduction="batchmean", log_target=False) + 0.5 * F.kl_div(
        q.log(), m, reduction="batchmean", log_target=False
    )
    return js
...
def polynomial_kernel(
    x: torch.Tensor, y: torch.Tensor, degree: int = 2, gamma: float = 1.0, coef0: float = 0.0
) -> torch.Tensor:
    """
