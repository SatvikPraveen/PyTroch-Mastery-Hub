# pytorch_mastery_hub/utils/metrics.py
"""
...
def classification_report(
    y_pred: torch.Tensor, y_true: torch.Tensor, class_names: list[str] | None = None
) -> dict[str, Any]:
    """
...
def confusion_matrix_torch(
    y_pred: torch.Tensor, y_true: torch.Tensor, num_classes: int | None = None
) -> torch.Tensor:
    """
...
def calculate_class_weights(y: torch.Tensor, num_classes: int | None = None) -> torch.Tensor:
    """
...
def pearson_correlation(y_pred: torch.Tensor, y_true: torch.Tensor) -> float:
    ...
    with torch.no_grad():
        ...
        denominator = torch.sqrt(
            ((y_pred - mean_pred) ** 2).sum() * ((y_true - mean_true) ** 2).sum()
        )
...
class EarlyStopping:
    ...
    def __init__(
        self, patience: int = 7, min_delta: float = 0.0, restore_best_weights: bool = True
    ):
