# pytorch_mastery_hub/computer_vision/augmentation.py
"""
...
class CopyPaste(nn.Module):
    ...
    def forward(
        self, img1: torch.Tensor, boxes1: torch.Tensor, img2: torch.Tensor, boxes2: torch.Tensor
...
class AdvancedAugmentationPipeline(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, y: torch.Tensor, phase: str = "train"
    ) -> tuple[torch.Tensor, torch.Tensor]:
        if phase != "train":
            return x, y
