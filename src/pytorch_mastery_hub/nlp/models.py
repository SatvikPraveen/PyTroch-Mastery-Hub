# pytorch_mastery_hub/nlp/models.py
"""
...
class RNNClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(nn.Dropout(dropout), nn.Linear(output_dim, num_classes))
...
class TransformerClassifier(nn.Module):
    ...
    def forward(
        ...
        emb = self.embedding(input_ids) * torch.sqrt(
...
class Encoder(nn.Module):
    ...
    def __init__(
        self, vocab_size: int, embedding_dim: int, hidden_dim: int, num_layers: int, dropout: float
    ):
        super().__init__()
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.lstm = nn.LSTM(
            embedding_dim + hidden_dim, hidden_dim, num_layers, dropout=dropout, batch_first=True
        )
        self.output = nn.Linear(hidden_dim, vocab_size)
