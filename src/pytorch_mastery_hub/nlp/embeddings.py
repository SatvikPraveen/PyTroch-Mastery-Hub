# pytorch_mastery_hub/nlp/embeddings.py
"""
...
class WordEmbedding(nn.Module):
    ...
    def load_pretrained(self, pretrained_weights: torch.Tensor):
        ...
        if pretrained_weights.shape != self.embedding.weight.shape:
            raise ValueError(
                f"Shape mismatch: expected {self.embedding.weight.shape}, got {pretrained_weights.shape}"
            )
...
def load_pretrained_embeddings(
    filepath: str | Path, word_to_idx: dict[str, int], embedding_dim: int, format: str = "glove"
) -> torch.Tensor:
    """
...
def create_embedding_matrix(
    word_to_idx: dict[str, int], pretrained_embeddings: dict[str, np.ndarray], embedding_dim: int
) -> torch.Tensor:
    """
...
class SubwordEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, words: list | None = None) -> torch.Tensor:
        # Word embeddings
...
def compute_embedding_similarity(
    emb1: torch.Tensor, emb2: torch.Tensor, metric: str = "cosine"
) -> torch.Tensor:
    """
